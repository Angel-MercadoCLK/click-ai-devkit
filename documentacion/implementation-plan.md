...
- Scoop bucket: dedicated `click-seguros/scoop-bucket` repo in the Click org (D18); GoReleaser `scoop:` block auto-commits the manifest on tag via a scoped deploy token — no manual step
...
- [ ] `scoop bucket add click-seguros https://github.com/click-seguros/scoop-bucket` then `scoop install click` succeeds on a clean Windows machine (integration test, windows-latest runner; D18)
