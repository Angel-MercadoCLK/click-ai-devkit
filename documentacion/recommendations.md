...
| D18 | Dedicated `Angel-MercadoCLK/scoop-bucket` repo, auto-published by CI on tag. | Keeps the manifest update mechanical (GoReleaser's `scoop:` block) rather than a manual step prone to drift from the actual release. | Low — standard pattern. Risk is entirely in the CI deploy token's scope and storage (flagged in `implementation-plan.md` Slice 6) — confirm it's a scoped, secret-store-held token, not a broad PAT. |
