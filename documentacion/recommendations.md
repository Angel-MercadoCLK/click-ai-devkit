...
| D18 | Dedicated `click-seguros/scoop-bucket` repo, auto-published by CI on tag. | Keeps the manifest update mechanical (GoReleaser's `scoop:` block) rather than a manual step prone to drift from the actual release. | Low — standard pattern. Risk is entirely in the CI deploy token's scope and storage (flagged in `implementation-plan.md` Slice 6) — confirm it's a scoped, secret-store-held token, not a broad PAT. |
...
- ✅ **Scoop command** corrected from the Homebrew-style `scoop install click-seguros/tap/click` to
  the proper two-step `scoop bucket add click-seguros <url>` + `scoop install click` in all six docs.
