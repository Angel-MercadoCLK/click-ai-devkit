...
| D41 | Token presence-only safety, never on argv or in logs | **Click reads only `ENGRAM_CLOUD_TOKEN` presence (`!= ""`); the value is never on argv, never logged, and never written to `engram-cloud.json` or any click state file.** It reaches the `engram` subprocess solely via inherited process env. Source + test (`TestSyncEngramCloud_TokenNotInArgvOrState`) verify the exclusions. Persisted token storage is handled by D46. Implemented in sdd/engram-cloud-wiring. | Superseded by D46 for persisted storage; prohibitions on argv and logs remain |
...
| D46 | Token may be persisted to click-owned settings.json with explicit consent | **The Engram Cloud token MAY be persisted to `~/.claude/settings.json` ONLY after explicit user consent, stored atomically with owner-only permissions (0600 on POSIX, owner-only DACL on Windows), and MUST be removed by `click uninstall`.** This relaxes D41's absolute prohibitions for a specific storage path: click owns `settings.json` and writes it atomically; the token is still never on argv, never logged, and never in `engram-cloud.json` or any other click state file. Rationale: Claude Code launched from a Dock icon or Start-menu shortcut inherits no shell profile, so requiring a shell-exported token would silently disable autosync for exactly the non-technical case the feature exists to serve. | Confirmed |
| D47 | Rollout gate (RISK-1b) partially validated against a real server; automatic-hook path still unproven | **A manual test against a real deployed Engram Cloud server (v1.20.0) confirmed the core bridge: a memory pushed by one enrolled client (`engram sync --cloud --project <p>`) was retrieved by a second, independent client via `engram sync --cloud --project <p> --import` + `engram search`.** This resolves the previously binary-symbol-only claim that the server bridges `/sync/mutations/push` into the `cloud_chunks` manifest consumed by import. It does NOT validate: the automatic `SessionStart`-triggered import with zero user action (the actual shipped hook path); repeated-import deduplication by a fixed `sync_id`; or multi-chunk import latency under the 5-second cap. The manual pre-rollout task in the change's task list remains unchecked and gates real developer rollout independently of this document/code being merge-ready. | Confirmed (partial) |
